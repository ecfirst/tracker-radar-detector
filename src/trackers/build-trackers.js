...
const newDataStats = JSON.parse(fs.readFileSync(`${this.sharedData.config.trackerDataLoc}/crawlStats.json`, 'utf8'))
let requestsArray = []

let chunk = 0
while (fs.existsSync(`${this.sharedData.config.trackerDataLoc}/commonRequests-${chunk}.json`)) {
    const requests = JSON.parse(fs.readFileSync(`${this.sharedData.config.trackerDataLoc}/commonRequests-${chunk}.json`, 'utf8'))
...
if (this.sharedData.config.crawlerDataLoc !== 'postgres') {
    const crawlMetadata = JSON.parse(fs.readFileSync(`${this.sharedData.config.crawlerDataLoc}/metadata.json`, 'utf8'))
    this.sharedData.config.regionCode = crawlMetadata.config.regionCode
}
const countryCode = this.sharedData.config.regionCode || 'US'
const crawledSiteTotal = newDataStats.sites
const summary = {trackers: 0, entities: []}

let requestPageMap = {}
try {
    requestPageMap = JSON.parse(fs.readFileSync(`${this.sharedData.config.pageMapLoc}/pagemap.json`, 'utf-8'))
} catch (e) {
...
if (!fs.existsSync(`${this.sharedData.config.trackerDataLoc}/domains/${countryCode}`)) {
    fs.mkdirSync(`${this.sharedData.config.trackerDataLoc}/domains/${countryCode}`)
}

for (const [fileName, tracker] of Object.entries(trackers)) {
    const filePath = `${this.sharedData.config.trackerDataLoc}/domains/${countryCode}/${fileName}`
    fs.writeFileSync(filePath, JSON.stringify(tracker, null, 4))
}

if (this.sharedData.config.includePages) {
    fs.writeFileSync(`${this.sharedData.config.pageMapLoc}/trackerpagemap.json`, JSON.stringify(trackerPageMap, null, 4))
}
...
if (!fs.existsSync(`${this.sharedData.config.trackerDataLoc}/build-data/generated/${countryCode}`)) {
    fs.mkdirSync(`${this.sharedData.config.trackerDataLoc}/build-data/generated/${countryCode}`)
}

fs.writeFileSync(`${this.sharedData.config.trackerDataLoc}/build-data/generated/${countryCode}/releasestats.txt`, `${summary.trackers} domains\n${summary.entities.length} entities`)

function log (msg) {
    if (this.sharedData.config.verbose) {
        console.log(msg)
    }
}
